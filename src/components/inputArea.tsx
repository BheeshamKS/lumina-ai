...
const Chip = ({ icon, label, onClick }: ChipProps) => (
  <button
    onClick={onClick}
...
interface InputAreaProps {
  ...
  setActiveModel: (model: ModelEntry) => void;
...
export const InputArea = ({
  ...
  const groupedModels = availableModels.reduce<
    Record<string, { provider: string; models: ModelEntry[] }>
  >((acc, model) => {
  ...
  const selectModel = (model: ModelEntry, provider: string) => {
    ...
    if (isRecording) {
      return (
        ...
        >
          ...
          <Square size={15} className="text-red-500 fill-red-500 relative z-10" />
        </button>
      );
    }
          ...
          <div className="flex justify-between items-center mt-1 px-1">
            ...
            <div className="flex items-center gap-2 md:gap-3 text-card-text text-xs font-medium">
              <div className="relative" ref={menuRef}>
                <div
                  onClick={() =>
                    !isRecording && !isTranscribing && setIsMenuOpen(!isMenuOpen)
                  }
                        ...
                        availableProviders.map((group) => (
                          <div key={group.provider}>
                            ...
                            {group.models.map((model) => (
                              <button
                                key={model.id}
                                onClick={() => selectModel(model, group.provider)}
